    ...
    <div class="bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      <div class="p-6 space-y-6">
        <div class="flex justify-between items-center">
          <h2 class="text-2xl font-bold text-primary">Ajouter un nouveau texte</h2>
          <button 
            @click="$emit('close')"
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="flex justify-end gap-4">
            ...
            >
              {{ loading ? 'Enregistrement...' : 'Ajouter le texte' }}
            </button>
          </div>
        </form>
...
async function handleSubmit() {
  try {
    ...
    // Émettre l'événement avec les données du nouveau texte
    emit('submit', data)
    emit('close')
    ...
    error.value = err.message || 'Une erreur est survenue lors de l\'ajout du texte'
  } finally {
    loading.value = false
  }
}
